#include "../include/nn/dendritic.h"
        ...
        for (img_x = 0; img_x < img_width; img_x += stride)
        {
            ...
            for(kernel_y = -kernel_med; kernel_y < kernel_med + 1; kernel_y++)
            {
                for(kernel_x = -kernel_med; kernel_x < kernel_med + 1; kernel_x++)
                {
                    ...
                    // Verifica se está em uma regiçao valida
                    if(
                        img_y_new > -1 && img_y_new < img_height
                        && img_x_new > -1 && img_x_new < img_width
                    )
                    {
...
{
    valid_x = position_pixel_x < max_position && position_pixel_x > min_position ? 1 : 0;
    valid_y = position_pixel_y < max_position && position_pixel_y > min_position ? 1 : 0;
    return valid_x && valid_y ? 1 : 0;
}